...
> **⚠️ 알려진 갭 (조장 PR #106 리뷰 반영)**: functional/industry는 seed `instructions`가 있을 때만 SkillDocument를 수집(선택)하므로, **② seed instructions 채우기 전까지 일부 seed 스킬은 SkillDocument(`skill_documents`)가 비어 있다.** SOP 경로는 완비(LLM 필수 생성), seed 경로는 부분 — Main Agent가 사용자에게 옵션 제시 시 SOP 스킬만 지침서가 있고 seed 스킬은 없는 비일관 UX가 ② 완료 전까지 가능. **의도된 갭**이며 후속 seed 작업으로 해소.
