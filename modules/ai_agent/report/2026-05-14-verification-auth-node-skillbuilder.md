...
### 🔵 docs PR (5/14 PR #51 머지 완료, 5/15 현재 진행 가능)

> PR #51은 5/14 머지 완료(`15d0f87`). 5/15 시점에는 "지금 바로 진행 가능" 상태. 5/15 추가 약속분 묶어서 단일 docs PR 1건으로 처리.

**EmbedderPort stale 정정 6곳** (5/14 박아름이 햄햄에게 약속한 후속) — `EmbeddingPort` → `EmbedderPort` (nodes_graph SSOT 정합):

| 파일                                   | 라인                                                              |
| -------------------------------------- | ----------------------------------------------------------------- |
| `CLAUDE.md`                          | 178 (예외 패턴 1줄 주석 추가 — ai_agent 외부 모듈 소유)          |
| `docs/specs/REQ-004-ai-agent.md`     | 95, 148, 149                                                      |
| `MONOREPO_STRUCTURE.md`              | 99, 408                                                           |
| `docs/context/clean_architecture.md` | 363, 1274, 1510                                                   |
| `docs/context/clean_architecture.md` | 903 (`SkillEmbedderPort` 별도 확인 — REQ-008 marketplace 영역) |

**CLAUDE.md "modules 간 허용된 교차 import" 표 추가 1줄** (5/15 햄햄 NodeSearchPort 답변에서 약속):

```
| toolset | nodes_graph의 application/use_cases | from nodes_graph.application.use_cases import SearchNodesUseCase |
```

**(선택) REQ-004 spec §2.2 Skills Builder 책임 경계 "옵션 A 확정" 명시** — 5/14 박아름 결정 spec 반영

→ **작업 규모**: 단일 docs PR 1건, 약 30분~1시간. 코드 변경 0건.
...
- 25종 명시는 spec에 없음 — 사용자 발언 출처 확인 필요
...
- **SSOT 분류 명칭 (실제 코드)**: `draft / refine / clarify / build_skill / propose` (5가지)
- **"장기적/단기적" 명칭은 SSOT에 없음** — 조장 발언 또는 추가 분류 검토 중 가능성
- **라우팅 분기 구현**: Orchestrator `RouteRequestUseCase` (신정혜 영역)

**SSOT 비교**:

- `route_request_use_case.py:7-10` 5가지 intent 분기 명시 ↔ REQ-004 spec §3.1 일치
- "장기/단기" 별도 분류는 spec / 코드 / CLAUDE.md 어디에도 없음 — 조장 SSOT 갱신 필요 시 박아름이 카톡으로 확인
...
| 8.3 skill builder (트리거 흐름 + 분기 + baseline)  | ✅ §8.3    | ✅ 일치 (25종 vs 11종 차이는 spec에 25 명시 없음) | baseline 25종 확장 필요 시 박아름 seed JSON 14종 추가          |
| 8.4 기타 (LangGraph + Frame + Modal + UI + 장단기) | ✅ §8.4    | ⚠️ "장단기" 명칭은 SSOT에 없음                  | 조장에게 "장단기 분류" 출처 확인 (Sprint 4 추가 분류 검토 중?) |
...
| # | 항목 | 현재 (stale) | 정정 제안 | 근거 |
|---|---|---|---|---|
| 1 | **"modules 간 허용된 교차 import" 표** — `auth → nodes_graph` 누락 | 표에 라인 없음 | `\| auth \| nodes_graph의 domain/ports \| from nodes_graph.domain.ports import NodeDefinitionRepository \|` 추가 | `credential_injection_service.py:6` import 실재 (§1.5) |
| 2 | **"modules 간 허용된 교차 import" 표** — `toolset → nodes_graph` 누락 | 표에 라인 없음 | `\| toolset \| nodes_graph의 application/use_cases \| from nodes_graph.application.use_cases import SearchNodesUseCase \|` 추가 | 햄햄 NodeSearchPort 협의 (5/15) |
| 3 | **"Port → Adapter 매핑" 표** — `EmbeddingPort` stale (line 178) | `ai_agent/domain/ports/EmbeddingPort` | `nodes_graph/domain/ports/EmbedderPort` (예외 패턴: 구현체는 `ai_agent/adapters/llm/` 소유) | PR #30 5/12 박아름 결정 |
| 4 | **카탈로그 종 수** (line 5 부근 "54종 노드 카탈로그") | 54종 | 56종 (gemma_chat 추가, PR #68) | 5/14 야간 결정 + PR #68 |
| 5 | **"Port → Adapter 매핑" 표** — 누락 항목 보강 | `CipherPort` / `ParserPort` 정합 / `EmbedderPort` 누락 | `EmbedderPort` 추가 + 예외 패턴 1줄 주석 | 햄햄 자료 한 줄 정리 정합 |
...
| 항목 | 상태 |
|---|---|
| **toolset_nodes.py 14종 제거 협의** (5/15 햄햄 카톡 발송) | ⏳ 햄햄 응답 대기 — 14종 전체 제거 vs 중복 3건만 제거 확정 부탁 |
| 중복 3건 동의: `slack_notify` / `http_request_tool` / `conditional·loop` | ✅ 박아름 동의, 시나리오 무관 toolset 쪽 제거 |
| **NodeSearchPort 위치** (`toolset/adapters/node_search_adapter.py`) | ✅ 박아름 동의 (5/15 답변 발송) |
| **NodeSearchPort 의존성 방향** (`toolset → nodes_graph application/use_cases`) | ⏳ 박아름 docs PR로 CLAUDE.md 갱신 처리 (위 9.1 항목 #2) |
| **한 줄 정리 보강** (햄햄 자료에 박아름 영역 비-DB Port 3건 누락) | ⏳ 박아름 docs PR로 처리. EmbedderPort 예외 패턴 명시 |
...
| **PR #68 리뷰 + 머지** (gemma_chat 신설) | 🟡 High | ⏳ 조장 approval 대기 |
| **baseline 25종 출처 확인** — 현재 spec/코드 = 11종 (functional 5 + industry 6), 25종은 어디서? | 🟢 Medium | ⏳ 조장 카톡 확인 필요 (§8.3 + §8.5) |
| **"장기적/단기적" 의도 분류 출처 확인** — SSOT 명시 없음, 실제 코드 = 5가지 | 🟢 Medium | ⏳ 조장 카톡 확인 필요 (§8.4 + §8.5) |
