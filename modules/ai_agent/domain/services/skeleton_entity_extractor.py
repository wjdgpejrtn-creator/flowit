...
_BRANCH_KEYWORDS: tuple[str, ...] = (
    ...
    # 단독 "분기"(branch)는 "분기 성과/매분기/분기별/분기말"(quarter, 시간단위)과 동음이의 →
    # 콘텐츠 발화를 branch 제어흐름으로 오인해 스켈레톤이 bail하던 문제. has_branch는 라우팅
    # 신호이므로 발화 콘텐츠 단어가 아니라 분기 **동작**(동사 분기하다/분기시키다 + "분기 처리")만
    # 둔다. 동사 어간은 공백 없이 붙고(분기하여/분기해서/분기한다/분기했다…), quarter는 항상
    # "분기 ○○"처럼 공백을 동반("분기 성과/분기 하반기/분기 한도")하므로 충돌하지 않는다.
    "분기하", "분기해", "분기한", "분기했", "분기시", "분기 처리",
    "조건에 따라", "라면 ", "이라면 ", "유형에 따라", "등급에 따라", "분류에 따라",
)
