...
class LangGraphOrchestrator:
    ...
    async def _validator_node(self, state: _State) -> dict:
        ...
        return {
            ...
            "collected_frames": [
                ...
                PipelineStatusFrame(service_name="validator", status="completed", elapsed_ms=0),
            ],
        }
    ...
    async def _qa_evaluator_node(self, state: _State) -> dict:
        ...
        return {
            ...
            "collected_frames": [
                RationaleDeltaFrame(delta=f"⭐ 품질 평가 완료 — 점수: {result.score}/10 ({status_text}) | {result.reason}"),
                QAMetricFrame(
                    score=result.score,
                    attempt=attempt,
                    pass_flag=result.pass_flag,
