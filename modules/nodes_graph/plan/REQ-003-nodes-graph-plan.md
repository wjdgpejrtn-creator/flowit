...
**담당자**: 박아름  
**작성일**: 2026-05-06  
**참조 스펙**: `docs/specs/REQ-003-nodes_graph.md` (development 브랜치)  
...
- 구현체(BGE-M3)는 ai_agent 모듈 담당 → 이 모듈에서 구현하지 않음
...
```
이 모듈 → common_schemas (WorkflowSchema, NodeInstance, Edge, ValidationErrorResponse 등)
이 모듈 ← auth (REQ-002): NodeDefinitionRepository.get_by_id() 후 NodeDefinition 필드 접근
이 모듈 ← ai_agent (REQ-004): GraphValidator 호출, SearchNodesUseCase 소비
이 모듈 ← execution_engine (REQ-007): 위상 정렬 실행 순서 결정
...
- [x] Ruff lint 통과
- [x] report 작성 (`modules/nodes_graph/report/`)
