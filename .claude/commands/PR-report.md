...
현재 브랜치를 확인하고, 아래 기준에 따라 경로를 결정한다.

| 현재 브랜치 | 변경 유형 | 경로 |
|------------|-----------|------|
| `feature/*` | REQ 기능 구현/변경 | **Full PR 경로** — Step 1~7 전체 수행 |
| `development` | 자잘한 수정 (문서, 설정, 오타, 린트) | **Direct Push 경로** — Step 1~3만 수행 후 `git push origin development`로 종료. PR/feature 브랜치 생성하지 않음 |
| `development` | REQ 기능 구현/변경 | `feature/req-xxx-*` 브랜치를 먼저 생성 후 Full PR 경로 진행 |

**자잘한 수정 판별 기준**: 변경 파일이 `*.md`, `.claude/`, `_agent_templates/`, `.gitignore`, 린트/포맷팅 수정만 포함하면 "자잘한 수정"으로 분류.
...
## 4. base 브랜치(development) 최신화

```bash
# 1) 원격 최신 상태 가져오기
git fetch origin

# 2) base 브랜치(development)와 현재 브랜치 간 diverge 여부 확인
git log HEAD..origin/development --oneline
git log origin/development..HEAD --oneline
```

- `origin/development`에 내 브랜치에 없는 커밋이 있으면 → **pull 먼저 수행**
...
PR base branch는 항상 `development`이다.
(`development` → `main` merge는 릴리즈 타이밍에 조장 또는 리포 오너가 별도 판단한다.)
