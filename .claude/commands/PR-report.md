...
**원칙**: 모든 변경은 PR 리뷰를 거친다. 별도 브랜치 생성 여부만 달라진다.

| 현재 브랜치 | 경로 |
|------------|------|
| `feature/*` | 현재 브랜치에서 그대로 커밋 → `development` PR (Step 1~7 전체 수행) |
| `development` | 현재 브랜치에서 그대로 커밋 → `development` 기존 PR 업데이트 또는 신규 PR 생성 (Step 1~7 전체 수행). **별도 feature 브랜치 생성하지 않음** |

**별도 브랜치를 만드는 경우**: 안정화/배포 단계에서 발생하는 버그 수정 시에만 `hotfix/*` 브랜치 생성. 개발 단계에서는 현재 브랜치에 그대로 커밋한다.
