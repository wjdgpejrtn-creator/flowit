...
describe('ConfirmCard', () => {
  it('explanation 있으면 의도/단계/권한 매니페스트를 렌더한다', () => {
    render(
      <ConfirmCard message="fallback" explanation={explanation} onExecute={jest.fn()} onEdit={jest.fn()} />,
    );
  ...
  it('explanation 없으면 fallback 메시지로 graceful degrade', () => {
    render(<ConfirmCard message="워크플로우가 완성됐습니다." onExecute={jest.fn()} onEdit={jest.fn()} />);
  ...
  it('가정 섹션은 토글로 펼쳐진다', async () => {
    ...
    render(<ConfirmCard message="f" explanation={explanation} onExecute={jest.fn()} onEdit={jest.fn()} />);
  ...
  it('실행/편집 버튼이 콜백을 호출한다', async () => {
    const user = userEvent.setup();
    const onExecute = jest.fn();
    const onEdit = jest.fn();
    render(<ConfirmCard message="f" explanation={explanation} onExecute={onExecute} onEdit={onEdit} />);
  ...
  it('loading 중에는 실행 버튼이 비활성화된다', () => {
    render(
      <ConfirmCard message="f" explanation={explanation} onExecute={jest.fn()} onEdit={jest.fn()} loading />,
    );
    expect(screen.getByText('실행 중…').closest('button')).toBeDisabled();
  });
});
