...
test('검토 & 게시: source_document_id + staging 전송 후 self-publish 체인 실행', async () => {
  ...
  // detail(instructions/staging) 도착 후에만 저장 가능 — 미완이면 버튼 disabled(빈 지침서 가드).
...
test('상세(detail) 추출 미완 상태에서는 생성이 차단된다 — 빈 지침서 방지 가드', async () => {
  // 회귀 가드: 카드를 선택하면 이름/설명은 즉시 차지만 instructions/staging은 비동기 detail로
  // 채워진다(콜드 시 수십 초). 그 사이 "검토 & 게시"를 누르면 instructions=undefined +
  // node_spec_staging=undefined로 저장돼 지침서 없는 placeholder 스킬이 만들어졌다(staging 실측 버그).
  // detail이 도착할 때까지 저장 버튼이 disabled여야 하고, 그래서 createPersonalSkill이 호출되지 않아야 한다.
  let resolveDetail: (v: unknown) => void = () => {};
  mockExtractDetail.mockReturnValue(new Promise((res) => { resolveDetail = res; }));
  mockStreamExtract.mockImplementation(async (_m: unknown, onFrame: (f: Record<string, unknown>) => void) => _draftFrame(onFrame));
  mockCreatePersonal.mockResolvedValue(CREATED);
  const user = userEvent.setup();

  window.history.pushState({}, '', '/skills/builder?source_document_id=doc-8');
  render(<SkillBuilderPage />);

  // 단건 자동선택 → 이름은 즉시 채워지지만 detail은 아직 pending(resolveDetail 미호출).
  await waitFor(() => expect(screen.getByPlaceholderText(/주간 리포트 자동화/)).toHaveValue('스킬 A'));
  const publishBtn = screen.getByRole('button', { name: '검토 & 게시' });
  expect(publishBtn).toBeDisabled();
  await user.click(publishBtn);
  expect(mockCreatePersonal).not.toHaveBeenCalled();

  // detail이 도착하면(staging 채워짐) 버튼이 활성화되고 정상 생성 가능.
  resolveDetail({ node_type: 'a', instructions: '## A', staging: STAGING });
  await waitFor(() => expect(publishBtn).not.toBeDisabled());
  await user.click(publishBtn);
  await waitFor(() => expect(mockCreatePersonal).toHaveBeenCalledTimes(1));
});

test('detail 추출 실패 시 버튼 disabled 유지 + "추출 중"이 아닌 실패 안내 노출', async () => {
  // 리뷰 LOW 가드: detail이 throw하면 staging이 영구 undefined → 버튼 영구 disabled인데,
  // 안내가 "추출 중"으로 남으면 무한 대기로 오인된다. 실패 시 재시도 안내로 분기되어야 한다.
  mockExtractDetail.mockRejectedValue(new Error('boom'));
  mockStreamExtract.mockImplementation(async (_m: unknown, onFrame: (f: Record<string, unknown>) => void) => _draftFrame(onFrame));
  mockCreatePersonal.mockResolvedValue(CREATED);
  const user = userEvent.setup();

  window.history.pushState({}, '', '/skills/builder?source_document_id=doc-10');
  render(<SkillBuilderPage />);

  await waitFor(() => expect(screen.getByPlaceholderText(/주간 리포트 자동화/)).toHaveValue('스킬 A'));
  // 실패 후: "추출 중" 문구는 사라지고 재시도 안내가 뜬다.
  await waitFor(() => expect(screen.getByText(/상세 추출에 실패했어요/)).toBeInTheDocument());
  expect(screen.queryByText(/상세\(지침서·입출력\)를 추출하는 중/)).not.toBeInTheDocument();
  // 버튼은 disabled 유지 → 빈 지침서 생성 차단.
  expect(screen.getByRole('button', { name: '검토 & 게시' })).toBeDisabled();
  expect(mockCreatePersonal).not.toHaveBeenCalled();
});
