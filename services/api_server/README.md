...
| | `PUT /auth/users/{id}/role` | REQ-002 GrantUserRoleUseCase — Admin이 역할/소속 팀 부여 (스킬 마켓플레이스 RBAC) |
| `nodes` | `GET /nodes/catalog` | REQ-003 NodeDefinitionRepository (56종 — gemma_chat 포함, PR #68 5/15) |
| `skills` | `POST /skills/{id}/approve`, `POST /skills/{id}/publish` | REQ-013 skills_marketplace ApproveSkill/PublishSkill — scope-aware 게시 lifecycle (ADR-0020 Q4). submit(DRAFT→REVIEW)은 `SubmitSkillUseCase` 신설 후 추가 |
