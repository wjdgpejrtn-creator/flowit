...
Clean Architecture에서 `modules/storage/`는 **Interface Adapter 계층**으로, 다른 모듈(`auth`, `ai-agent`, `nodes-graph`, `toolset`, `doc-parser`)이 `domain/ports/`에 정의한 Repository ABC를 구현하는 역할이다.
...
### Follow-ups
- `services/api-server/`에서 DI 컨테이너로 Repository 주입 구현 필요 (REQ-009)
- `services/execution-engine/`에서 동일하게 DI 조립 필요 (REQ-007)
