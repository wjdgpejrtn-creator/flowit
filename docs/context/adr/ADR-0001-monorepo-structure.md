...
2. **원자적 변경 불가**: API 스키마 변경 시 api_server·frontend·common_schemas를 각 브랜치에서 별도 PR로 처리해야 함
...
| 디렉토리 | 역할 | 대응 REQ |
|---|---|---|
| `packages/` | 공유 패키지 (common_schemas) | REQ-012 |
| `services/` | 배포 가능 서비스 (api_server, execution_engine, frontend) | REQ-007, 009, 010 |
| `modules/` | 도메인 모듈 (auth, nodes_graph, ai_agent, toolset, doc_parser, storage) | REQ-002~006, 008 |
...
- **타입 안전성**: `packages/common_schemas`를 Python·TypeScript 모두 단일 소스에서 참조
