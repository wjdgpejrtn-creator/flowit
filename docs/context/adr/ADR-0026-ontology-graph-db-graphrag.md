...
- **Status**: Accepted (Phase 0/1/2a/2b 구현·머지 + §6.5 하니스 검증 완료, 2026-06-08). **§6 고도화 방향 전환**: soft 모티프 힌트 효과 0 측정(#416) → **§6.6 결정적 스켈레톤 라이브러리**로 전환(plan §6.6, Modal 복귀 후 구현). #418 범용 LLM 노드 always-include 선구현(qa_pass 0.45→0.75).
...
// §6.6 — 결정적 스켈레톤 (soft :Pattern 대체, 2026-06-08 전환). 상세 plan §6.6.2
(:Skeleton {name, intent_keywords})                    // scheduled_pipeline / event_response / quality_loop
(:Skeleton)-[:HAS_SLOT {order, role, required, cardinality}]->(:SlotSpec)  // 데이터흐름 trigger/source/transform/scorer/sink/gate + 제어 router/splitter/merger/delay/terminal
(:SlotSpec)-[:FILLED_BY {by}]->(:Node)                 // by=category|lexical|entity — 슬롯 후보 node_type
```

- **quality_loop 슬롯 체인 = generator(transform, ai) → scorer(llm_judge, score:number) → gate(if_condition, gte) → [false] back-edge로 generator 재생성 / [true] sink** (#438 §6.6 C, PR #443). scorer는 gate가 비교할 점수를 내는 1급 역할 — 이전엔 점수 내는 노드가 카탈로그에 없어 게이트가 의미 비교 불가였다(`llm_judge` 노드 #442로 신설). scorer는 ai 노드라 condition이 아니며, 발화 추출 대상이 아닌 `required+default` 구조 슬롯이다(loop_list 동류). SCC={generator, scorer, gate}에 condition(gate) 포함 → CyclicScheduler 수용(Phase 0 #392).
...
### 측정 결과 보강 (2026-06-08, 양팔 de-noise — 방향 전환의 근거)
- **soft `:Pattern` 모티프 힌트 효과 = 0** (branch_on_classification motif-correctness ABSENT 0.50 → PRESENT 0.50, robust null; #416). **작은 LLM은 프롬프트 soft 힌트로 구조를 안 바꾼다** → §6.1 soft 시드 접근 폐기.
- **진짜 레버 = 결정적 노드 강제 포함** (#418 범용 LLM 노드 always-include): 끊긴 워크플로우율 **23.2%→0.0%**, qa_pass **0.452→0.750**, qa_score 6.69→8.71 (OFF×3 de-noise vs ON, 효과가 std의 ~9배). 단 LLM 노드 한정.
- **잔여 갭(e2e 실측)**: 발화에 명시된 도메인 노드("광고 시트"→`google_sheets_read`)가 의미검색 미진입→drafter 누락→qa 루프 반복. → **§6.6 결정적 스켈레톤 라이브러리로 전환**(엔티티추출→슬롯 결정적 채움→코드 배선, LLM은 파라미터만). plan §6.6.
- **de-noise 방법론 부채**: 단일 공유 세션이 Cloud SQL 연결 끊김 시 트랜잭션 오염, staging oauth 스키마 드리프트(account_id 컬럼, 026 미적용)가 autobind 오염 → run_eval를 시나리오당 세션 + autobind off로 강건화(development 반영). 마이그레이션 026 staging 적용 완료.
...
- **§6 고도화 방향 전환 (2026-06-08 측정 후 확정)**: 의도된 고도화는 "retry·분기·fan-out 등 다양한 control-flow를 자연어로 1급 생성"이나, **soft 모티프 힌트가 효과 0으로 판명**(#416)되어 접근을 전환한다 → **§6.6 결정적 스켈레톤 라이브러리**: `:Pattern`을 슬롯 가진 `:Skeleton`(Neo4j)으로 확장하고, composer가 **발화 엔티티 추출 → 슬롯 결정적 채움(명시 도메인 노드 강제 진입) → 코드 배선**, LLM은 파라미터만 채운다. 트리거/싱크는 보편 슬롯, 소스/AI/검증게이트는 발화 함의 시 활성(조건부 필수 — "5요소 항상 필수"는 과강제라 정정). §6.1 role 매핑은 §6.6에 계승. 각 스켈레톤은 §6.5 하니스 + 엔진계약 파리티 게이트 필수. 별건 skill-binding 정밀화(무관 스킬 오바인딩)는 REQ-013 박아름 협의. 주도=신정혜(composer grounding)+황대원(그래프 스키마/ETL), **Modal 복귀 후 구현·측정**.
