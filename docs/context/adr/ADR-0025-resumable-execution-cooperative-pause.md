...
- 동시성 — **status clobber 방지가 핵심**(#380 리뷰 HIGH): `save()`의 `ON CONFLICT DO UPDATE SET status=EXCLUDED.status`는 status를 무조건 덮어쓴다. step별 체크포인트가 `save()`를 쓰면, pause task가 **step 실행 도중** 별도 트랜잭션으로 쓴 PAUSED를 그 step 종료 시점의 `save(RUNNING)`이 되돌려(clobber) pause 감지가 사실상 항상 유실된다(step이 길수록 창이 커짐). 따라서 step 체크포인트는 **status를 건드리지 않는 전용 경로 `save_checkpoint()`**(`node_results`만 UPDATE)로 분리한다. status 권한은 (시작 save=RUNNING / pause task=PAUSED / 종료 save=COMPLETED·FAILED·PAUSED)만 갖는다. 추가로, **마지막 step 실행 도중** 도착한 pause는 top-check가 못 잡으므로(다음 step 없음) 루프 종료 후 `_is_pause_requested`를 한 번 더 확인해 PAUSED면 완료 마킹하지 않는다. cancel(`revoke`)은 기존대로 즉시 강제 종료 경로 유지(`save_checkpoint`가 status를 안 쓰므로 cancel 상태도 clobber하지 않음).
...
- 엔진: `services/execution_engine/src/application/use_cases/execute_workflow.py`(시작 save + step별 `save_checkpoint` + top-check & 루프후 `_is_pause_requested` + `_load_checkpoint`(NotFoundError만 graceful, 그 외 전파) + step skip), `adapters/postgres_execution_repo.py`(`save_checkpoint` = node_results-only UPDATE), `domain/ports/execution_repository_port.py`(`save_checkpoint` ABC), `adapters/celery_tasks.py`(`pause_execution_task` + task명 전부 `broker_tasks` 상수화).
