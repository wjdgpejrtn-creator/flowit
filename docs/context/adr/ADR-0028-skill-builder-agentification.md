...
- **개정 이력(2차)**: 2026-06-11 — **O1 RESOLVED(조장): (A) 결정적 파이프라인 채택** — agentic LLM tool-calling 루프 미신설, T1~T5는 고정 순서 결정적 파이프라인. GAP3(§6.6↔tool-calling 충돌) 해소. **O7 RESOLVED + 김진형(REQ-006) 통지 완료**(이슈 #493). **PR #492 머지**(development).
...
### O1. (조장) tool-calling 프레임워크 — ✅ **RESOLVED (2026-06-11 조장): (A) 결정적 파이프라인 채택**
