...
- **개정 이력(2차)**: 2026-06-11 — **O1 RESOLVED(조장+신정혜 합의): (A) 결정적 파이프라인 채택** — agentic LLM tool-calling 루프 미신설, T1~T5는 고정 순서 결정적 파이프라인. GAP3(§6.6↔tool-calling 충돌) 해소. 신정혜 PR #492 리뷰 통과 + O10 세 전제 + O3 composer read 수용. **O7 RESOLVED + 김진형(REQ-006) 통지 완료**(이슈 #493). **PR #492 머지**(development).
...
- ⚠️ **GAP 1 (조장 리뷰 HIGH) — 렉시컬 취약성 재현 위험**: composer는 ADR-0026 Phase 2(#463/#465)에서 SOURCE/SINK 선택을 **렉시컬 손사전 → 4-voter 의미 앙상블**로 옮겼다(qa_pass 50→82%, 사유=*"렉시컬이 못 따라잡는 발화 어휘 변형"*). **SOP 문서는 발화보다 어휘 변형이 더 크므로**, T4가 순수 키워드 매칭을 그대로 쓰면 composer가 막 폐기한 취약성을 스킬빌더가 재현한다. → **해소안(택1, 머지 후 Phase 1 보정)**: ① composer `_slot_resolver`(의미 앙상블) 재사용 검토(slot-filling 파일 동시편집 충돌 주의 — 신정혜 영역), 또는 ② "SOP는 키워드 적중이 많아 순수 매칭으로 충분"하다는 **근거 측정 + known-fragility 폴백**(미매칭 시 LLM)을 명시. 현 T4는 미매칭 시 LLM 폴백이 이미 있으나, 앙상블 정렬 여부는 신정혜 합의 대상.
...
### O1. (조장) tool-calling 프레임워크 — ✅ **RESOLVED (2026-06-11 조장+신정혜 합의): (A) 결정적 파이프라인 채택**

**결정**: agentic LLM tool-calling 루프를 **신설하지 않는다.** T1~T5는 진입 라우터/api_server가 **고정 순서로 호출하는 결정적 파이프라인**으로 두고, LLM은 리프(파라미터·instructions·detail 보강)에서만 쓴다. agentic AI 4요소(계획/도구사용/기억/반영) 중 **LLM 자율 planning·tool-calling은 제외**한다.

**근거**:
1. 현 `BuildFromSOPUseCase`가 이미 `extract_metadata → extract_detail(T4 스켈레톤 검색 + T5 결정적 조립) → confirm` **고정 3-step 위저드**이고 LLM tool-calling 0건 — 조장 확인 *"이미 3-step wizard 구조가 결정론적이었다."*
2. **GAP3 해소** = §6.6/#416 측정(*"작은 LLM은 힌트로 구조를 못 만든다"*)은 곧 *"Gemma에게 5-툴 루프 제어흐름(어떤 툴을·언제·종료 판단)을 맡기는 것도 동일 위험군"* 임을 함의한다. 힌트로 구조도 못 짜는 모델에 제어흐름을 위임할 수 없다 → **결정적 파이프라인이 정답.**
3. 빌드순서 1~3(T3→T4→T5)이 이미 결정적 파이프라인이라 자연스러운 귀결 → **"agentic LLM 루프" 프레임 자체가 불필요**(GAP3가 제기한 *결정적 파이프라인 vs agentic 루프* 1급 선택지 중 **결정적 파이프라인** 채택).
4. **(B) 순효용 음수 (신정혜 확인)**: T1~T5가 `parse→search→extract→assemble→map` **자연 순서**라 동적 툴 선택이 불필요하고, ReAct 프레임 신설은 composer/orchestrator까지 건드리는데 **얻는 게 없다**. → 나중에 고정 순서로 못 푸는 **동적 분기가 실제로 생기면 그때 (B) 재검토**.

**잔여(wrap 아님 — 진입 배선)**: T3→T4→T5는 `BuildFromSOPUseCase`에 이미 결정적 배선됨(라우터 `_stream` step 분기 고정 호출). T1(storage 어댑터=조장 후속)·T2(현재 api_server가 doc_parser로 파싱해 `payload["document"]` 주입 — 라우터 결정적 배선 여부는 api_server 경계 합의)만 진입점 배선 잔여. **GAP2(SSE/지연 교훈)는 결정적 파이프라인에도 그대로 적용** — T2 콜드 파싱·long step 구간은 #214 relay 패스스루 / #305·#307 15s heartbeat + GCS 세션 규약을 상속한다.

<details><summary>검토된 선택지 (아카이브 — 결정 전 원안)</summary>
...
- ⚠️ **GAP 3 (조장 리뷰 MEDIUM) — §6.6 ↔ tool-calling 정면 충돌**: §6.6 결정적 전제는 *"작은 LLM은 힌트로 구조를 못 만든다(#416)"*. 그런데 tool-calling 루프는 Gemma에게 **제어흐름 결정**(어떤 툴을·언제·종료 판단)을 시키는 **동일 위험군**이다. **핵심 질문**: *Gemma가 힌트로 구조를 못 짜면 5-툴 루프 제어흐름은 신뢰 가능한가?* 아니라면 **결정적 툴 파이프라인(현 빌드순서 1~3 = T3→T4→T5 고정) + 리프에서만 LLM**이 더 맞고 "agentic LLM 루프" 프레임 자체가 불필요할 수 있다(빌드순서 1~3이 이미 결정적 파이프라인이라 자연스러운 대안). → O1 결정에 이 트레이드오프(결정적 파이프라인 vs agentic 루프)를 1급 선택지로 포함. **→ 본 트레이드오프가 (A) 결정적 파이프라인으로 귀결(위 RESOLVED).**
- ⚠️ **GAP 2 (조장 리뷰 MEDIUM) — 멀티에이전트 SSE/지연 교훈 상속**: 5-툴 ReAct + T2 doc 파싱(콜드 수십초)은 긴 침묵 구간을 만든다. composer 멀티에이전트가 피흘린 교훈을 **재발견 금지·상속**으로 명문화: **#214**(supervisor relay `astream(stream_mode="updates")`+dict 반환이 SSE 패스스루 깨뜨려 100~300s 침묵) / **#305·#307**(drafter 무음 50~70s SSE drop → **15s heartbeat** producer/queue 필수 + 재연결은 **GCSSessionFrameStore**, in-memory 금지=멀티컨테이너 404). D5 "챗 inline 렌더"도 이 SSE 규약 위에 있다.
...
- **(신정혜 2026-06-11)**: 조장이 coarse→정밀 BINDS 전환의 **착수 주체·시점**을 정하면, 그 정밀 BINDS를 **composer가 read하는 경로는 신정혜(Composer 오너)가 받는다**. → 조장 결정만 남음.
...
- **합의 진척 (신정혜 2026-06-11)** — O10 확장은 **세 전제**로 간다: ① **벡터는 pgvector 유지**(Neo4j는 그래프 확장 전용, 스킬후보 임베딩 검색은 pgvector 계속) ② **O8 스키마 먼저**(도메인 ground truth 라벨/관계 확정 후 포트 시그니처 설계) ③ **`SKELETONS`는 코드 SSOT**(Neo4j 미러는 투영일 뿐 쿼리 대상 아님). → 세 전제 확정 후 `retrieve_domain_skeleton(...)` 신설(O8 선행).
...
4. **(조장 2026-06-11) O1 = (A) 결정적 파이프라인 확정 → 에이전트 루프 wrap 취소.** agentic 프레임을 신설하지 않는다. T3→T4→T5는 `BuildFromSOPUseCase`에 이미 결정적 배선(라우터 `_stream` step 분기 고정 호출, LLM tool-calling 0건). **T1~T5 콜러블 use case 전부 완료.** 잔여 = wrap이 아니라 **T1/T2 진입 배선**(T1 storage 어댑터=조장 후속 / T2 api_server 파싱 주입 또는 라우터 결정적 배선=경계 합의). GAP2 SSE/지연 규약은 결정적 파이프라인에도 상속.
   - ⚠️ **GAP 4 (조장 리뷰 LOW, wrap/배포 시 체크)**: ① **배포 패키징** — T2가 첫 `ai_agent → doc_parser` import. 스킬빌더를 Modal 서브앱 배포 시 **doc_parser를 이미지에 번들** 필수(#384 재발 위험 — 크로스모듈 의존 누락 시 런타임 `ModuleNotFoundError`, 단위테스트 미검출, executions 조용한 전멸). ② **Modal 엔진** — T1 storage 어댑터(조장 후속)가 Modal ASGI에서 pgvector 쿼리 시 **#261**(per-request NullPool 엔진, asyncpg loop-binding) 패턴 상속. ③ **T2 `execute` sync**(파일 I/O) — O1 async 루프에서 직접 호출 시 이벤트 루프 블로킹 → **threadpool offload** 필요(현재 콜러블이라 무해). ④ LangGraph tool node 채택 시 `recursion_limit`은 **runtime config 키**(compile 인자 아님 — TypeError).
