...
### O7. (박아름/김진형) T2 doc_parser 크로스모듈 import 신설 (조장 리뷰 #464 보강)
T2 `parse_document`는 **ai_agent → doc_parser `ParserPort`(domain/ports) import를 신설**한다. 규칙상 허용(domain/ports 참조)이나, 구현 시 **CLAUDE.md "modules 간 허용 import" 표에 행 추가 + doc_parser 오너(김진형, REQ-006) 통지** 필요. (T2가 doc_parser를 직접 호출할지, api_server composition root 경유 주입할지도 함께 결정.)
