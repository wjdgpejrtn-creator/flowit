...
- ✅ `SkillDocumentStore` GCS adapter 구현 = `storage/adapters/gcs_skill_document_store.py` (PR #160). `save()→str(gs:// URI)` 반환 — bucket 이름이 호출부로 누수되지 않도록 어댑터가 forward. SKILL.md = YAML frontmatter(name/description) + markdown body(instructions). 키: `skills/{skill_id}/SKILL.md`. production = `SKILLS_MARKETPLACE_BUCKET` env(전용 버킷).
