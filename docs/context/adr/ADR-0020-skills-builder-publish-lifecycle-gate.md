...
| **Q8** 추출 편집 wizard | **wizard 1차 채택 + 2단계 분리(옵션 1)** (2026-05-22 조장 입장 변경 + 2026-06-04 박아름·조장 LLM 토큰 최적화). 사용자가 추출된 노드 스펙(이름/설명/스키마)을 검토·수정 후 확정하는 wizard 구조. ③ `BuildFromSOPUseCase`를 **`extract_metadata`(메타 5필드만 추출 — node_type/name/description/category/risk_level, 카드 그리드용) + `extract_detail`(선택된 메타 → 그 노드의 inputs/outputs/instructions/required_connections/service_type 추출 + `NodeSpecStaging` 반환, 폼 prefill용) + `confirm`(편집 반영 → `CreateDraftSkillUseCase` DRAFT 저장)** 3메서드로 분리 + frontend 다단계 검토 UI(Step 2 메타 → Step 4 선택 → Step 4.5 detail → Step 5/6 편집). lifecycle 게이트(§1)는 그대로 — wizard 확정 후 DRAFT부터 lifecycle 진행. ※ "personal in-place"(lifecycle 승인)와 wizard(추출 편집)는 별개 축이며 **둘 다 1차** | (5/22) 사내 프로세스 안정 기업엔 불필요할 수 있으나, **스타트업·소규모는 wizard로 우리가 설계한 skill 구조를 확인하며 프로세스를 쌓아올리는 방향이 필요**(조장). one-shot은 검토 없는 확정이라 구조 학습/검증 기회 상실. staging(Q1)·`CreateDraftSkillUseCase`(②e) 기반이 이미 있어 wizard 백엔드 토대 확보. (6/4) 1회 호출에 노드 N개 × (긴 inputs/outputs JSON Schema + instructions markdown) 전체를 받아 `max_tokens=4096`을 초과해 JSON EOF 잘림 발생(라인 220~250 col 부근). 옵션 1(완전 분리)로 응답당 토큰 자체를 줄이고 사용자 선택 후 detail을 별도 호출하는 흐름으로 전환. `_STRUCTURED_MAX_TOKENS` 4096→8192는 안전망(박아름 PR, agent-skills-builder + composer + orchestrator + personalization 4 agent 공유 상수). API: `POST /skills/extract`(SSE, `payload.skill_metas`) + 신규 `POST /skills/extract/detail`(JSON, `payload.skill_detail`) |
...
- **frontend** (REQ-010) — Skill Builder 페이지: **추출 결과 검토·수정 wizard(Q8, 1차 + 옵션 1 2단계 분리, 2026-06-04)** + personal 생성 + in-place 검토(DRAFT→PUBLISHED) UI. Step 2 메타 추출 SSE(`POST /skills/extract`, `payload.skill_metas` 카드 그리드) → Step 4 사용자 선택 → Step 4.5 detail 추출 JSON(`POST /skills/extract/detail`, `payload.skill_detail` 폼 prefill) → Step 5/6 편집·미리보기 → Step 7 `POST /skills/personal` DRAFT 저장. team/company는 별도 Marketplace promote 흐름(Q3 promotion-only — Skill Builder에 team/company 직접 생성 UI 없음).
