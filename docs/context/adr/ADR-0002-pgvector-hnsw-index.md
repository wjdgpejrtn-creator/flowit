...
REQ-001 Database 구현에서 BGE-M3 768차원 벡터 임베딩을 저장하는 테이블이 4개 존재한다:
...
- 768차원 벡터에 대한 메모리 사용량: 노드당 ~49KB (m=16 기준)
...
- BGE-M3 모델: 768차원 dense embedding (임베딩 차원 변경 시 storage ORM의 Vector 컬럼도 반드시 동기화 필요)
