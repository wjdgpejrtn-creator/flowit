...
### L2 — 조건 분기 (live-edge reachability)
- condition 노드(`if_condition`/`switch_case`)의 출력이 **어느 출력 핸들(분기)이 live인지** 결정. 엣지의 `from_handle`이 분기 키.
- 레벨 일괄 실행을 **ready-queue 동적 실행**으로 전환: 노드는 *live한 incoming 엣지*로 도달 가능할 때만 실행. 안 탄 가지의 하류는 `skipped`. (비순환 유지.)
