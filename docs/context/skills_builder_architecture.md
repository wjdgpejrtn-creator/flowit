...
> 📌 **별도 DB 아님**: composer와 skill-builder는 **같은 Neo4j 인스턴스 1개**를 공유하고, `skeleton_library.SKELETONS`가 양쪽 SSOT다. "composer 쪽 그래프 DB"는 별도 저장소가 아니라 *composer가 이미 쓰는 조회 경로*를 뜻한다 — COMPOSER.md의 node_type 어휘가 composer 소비와 정합하려면 이 SSOT를 반드시 공유해야 한다.
