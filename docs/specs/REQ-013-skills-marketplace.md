...
| `SkillDocumentStore` | `save(skill_id, document) → str (gs:// URI)`, `load(skill_id) → SkillDocument \| None` | `storage/adapters/GcsSkillDocumentStore` (조장, PR #160 — 2026-05-24 협의 "md 문서 저장이라 storage" 확정). 기존 `ObjectStoragePort`(GCSAdapter/LocalStorageAdapter) 조합. SkillDocument(markdown) GCS 저장. Port는 skills_marketplace 소유. `save` 반환 gs:// URI를 호출부가 `skill_document_uri`에 세팅(bucket이 use case에 누수 안 되게 어댑터가 forward) |
