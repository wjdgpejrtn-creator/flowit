...
| `NodeSpecStaging` (VO) | `category`, `input_schema`, `output_schema`, `risk_level`, `required_connections`, `service_type` | publish 전 노드 스펙 임시 보관 (ADR-0020 Q1). **ADR-0024 D2: PUBLISHED 시 `NodeDefinition` 생성 폐기 — staging 컬럼은 잔존하나 미사용(스킬이 노드로 둔갑하던 #372 결함 B 차단)** |
...
|-----------|----------------|------|
| `CreateDraftSkillUseCase` | `owner_user_id, name, description, node_spec_staging, embedding?, skill_document_uri?, instructions?, composer_instructions?, source_document_id? → UUID` | Skills Builder(③) wizard `confirm` 단계가 추출+편집 결과를 personal DRAFT로 생성 (ADR-0020 ②e + 옵션 1 2단계 분리). NodeDefinition 미생성(① 무관), 노드 스펙은 `node_spec_staging` 보관(`extract_detail`이 staging 변환 후 frontend로 전달, confirm이 그대로 사용). `instructions`(SKILL.md 본문)·`composer_instructions`(COMPOSER.md 본문, ADR-0024 2-md) 중 하나라도 주어지고 `SkillDocumentStore`가 주입돼 있으면 GCS 저장 — skill_id 생성 후 GCS save(SKILL.md 항상, COMPOSER.md는 비어있지 않을 때) → 반환 URI를 `skill_document_uri`에 세팅(둘 다 없으면 문서 미저장, 하위호환) |
...
| `PublishSkillUseCase` | `skill_id, scope, *, actor_user_id, actor_role, actor_department_id` (생성자 +`node_def_repo` Optional deprecated, +`embedder` Optional, +`ontology_projector` Optional) | 게시 APPROVED → PUBLISHED. `SkillApprovalPolicy` 인가(위임2) + 검색용 임베딩 백필. **ADR-0024 D2(#372 결함 B): `NodeDefinition` 생성·upsert 폐기 — `node_definition_id` 미연결. 스킬은 실행 노드가 아니라 LLM 노드 주입 지침서(모델 A)이며 스킬 자체 임베딩(`SearchSkillsUseCase`)으로 검색. `node_def_repo`는 Optional 미사용(후속 제거)** **ADR-0026 Phase 2b: 게시 시 `ontology_projector`(`SkillOntologyProjector`)로 `(:Skill)-[:BINDS]->(:Node)` Neo4j incremental upsert(ai 노드 + `required_connections` 노드, 모델 A) — 미주입/실패 시 non-fatal(게시 유지, GraphRAG만 미투영). api_server DI는 `NEO4J_URI` 설정 시에만 `Neo4jSkillProjector` 주입.** scope별 owner/team 격리 |
...
| personal 미리보기/편집 api_server 라우트 (조장) | 4 엔드포인트(`GET /personal`, `GET/PUT/DELETE /personal/{id}`) + `PersonalSkillResponse` 슬림 DTO + `GetPersonalSkillUseCase`(조장 신설, owner-검사 라우터 분산 방지) | ✅ PR #195 |
