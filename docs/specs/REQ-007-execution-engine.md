...
| `NodeExecutorPort` | `execute(node: NodeInstance, config: NodeConfig, inputs: dict[str, Any], context: NodeContext) -> dict[str, Any]` | 개별 노드 실행 — node_type → `BaseNode.process(input, context)` 직접 호출 (ADR-0018) |
...
> **노드 실행 가능 범위 (ADR-0018)**: `CatalogNodeExecutor`는 53종을 동일 경로(`node_type → BaseNode.process()`)로 호출한다. **Phase 3d 완료로 53종 전부 `process()` 실구현** — domain 28종 + external 25종 모두 실행 가능. external은 transform/api 6종 + messaging 3종 + LLM/Linear 3종 + DB 3종(`postgresql/mysql/bigquery_query`) + Google 5종(`gmail_send`·`google_*`) + file 3종 + `http_request`·`pdf_generate`. `NotImplementedError` 스텁은 남아 있지 않다.
...
17.         -> 레벨 완료 대기
18.      -> ExecutionRepositoryPort.save(result)
19.      -> EventPublisherPort.publish_status(COMPLETED)
```
...
| 6 | `adapters/catalog_node_executor.py` | node_type → BaseNode.process() 실행 (ADR-0018, MVP 필수) |
| 7 | 나머지 어댑터 (persistence, credential, SSE) | 인프라 레이어 완성 |
