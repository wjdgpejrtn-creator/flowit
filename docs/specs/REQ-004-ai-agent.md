...
| `UpdateUserMemoryUseCase` | 워크플로우 완료 후 LLM이 패턴 추출 → 변경된 .md만 선택 저장 | `user_id: UUID`, `turn_count: int`, `session_summary: str\|None`, `workflow: WorkflowSchema\|None` | `bool` (저장 여부) | `LLMPort`, `PersonalMemoryStore.load_index()`, `save_file()`, `save_index()` — `turn_count < 3` 또는 `workflow.nodes` 없으면 early return |
