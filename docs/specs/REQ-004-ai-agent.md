...
|-----------|------|-------------|-------------------|--------------|
| `LLMPort` | LLM 호출 추상 인터페이스 | `generate(prompt: str, **kwargs) -> str`, `generate_structured(prompt: str, schema: type[T], max_tokens: int \| None = None) -> T` (max_tokens=출력 토큰 예산, 생략 시 구현체 기본값 2048 — SOP 추출 등 장문 출력은 호출부에서 상향) | `adapters/llm/` (Modal GPU) | 전체 |
