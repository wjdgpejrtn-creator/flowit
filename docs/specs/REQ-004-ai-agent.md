...
| 유스케이스 | 설명 | 입력 | 출력 | 호출하는 서비스/포트 |
|-----------|------|------|------|---------------------|
| `BuildFromSOPUseCase` | SOP 문서(DocumentBlock) → LLM 추출 → **wizard 2단계**(ADR-0020 Q8): `extract_draft`(추출 → `NodeSpecStaging`+메타 반환, **저장 X**) / `confirm`(편집 결과 → `CreateDraftSkillUseCase`로 personal DRAFT). NodeDefinition은 **publish 시점 생성**(Option B, ②d). `instructions`(SkillDocument 본문)는 extract_draft가 payload 반환, **confirm이 `CreateDraftSkillUseCase.execute(instructions=)`로 전달 → `SkillDocumentStore` 주입 시 GCS 저장**(PR #164/#165) | `extract_draft(user_id, document, personal_memory?)` / `confirm(user_id, skills)` | `AsyncGenerator[SSEFrame]` | `LLMPort`, `EmbedderPort`, **`CreateDraftSkillUseCase`(skills_marketplace)** |
