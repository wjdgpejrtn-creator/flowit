...
| `UpdateUserMemoryUseCase` | 워크플로우 완료 후 LLM이 패턴 추출 → 새 .md 작성/갱신 | `user_id: UUID`, `session_summary: dict`, `workflow: WorkflowSchema` | `None` | `LLMPort`, `PersonalMemoryStore.save_entry()`, `EmbedderPort.embed()` |
| `RecallPersonalSkillsUseCase` | Composer가 prompt 작성 시 호출 — 관련 personal skill 검색 | `user_id: UUID`, `query: str`, `limit: int` | `list[PersonalSkill]` | `PersonalMemoryStore.list_entries()`, `EmbedderPort.embed()` (코사인 유사도 매칭) |
