...
| `IntentResult.intent` | `common_schemas.agent` | (위 IntentResult import에 포함) | `Literal["clarify","draft","refine","propose","build_skill"]` — Sprint 3 `build_skill` 추가됨 |
...
| `EmbedderPort` (SSOT: `nodes_graph.domain.ports`) | 임베딩 호출 추상 인터페이스 (BGE-M3 768d) — ai_agent 자체 `EmbeddingPort` 폐기, nodes_graph SSOT 사용 | `embed(text: str) -> list[float]`, `embed_batch(texts: list[str]) -> list[list[float]]` | `adapters/llm/modal_embedding_adapter.py` (`nodes_graph.domain.ports.EmbedderPort` 구현) | composer, personalization |
...
| 유스케이스 | 설명 | 입력 | 출력 | 호출하는 서비스/포트 |
|-----------|------|------|------|---------------------|
| `BuildFromSOPUseCase` | SOP 문서(DocumentBlock) → SkillNode 목록 → nodes_graph 카탈로그 upsert | `user_id: UUID`, `document: DocumentBlock`, `personal_memory: list[MemoryEntry] \| None = None` | `AsyncGenerator[SSEFrame]` | `LLMPort`, `EmbedderPort`, `NodeDefinitionRepository.upsert()`(nodes_graph) |
| `BuildFromIndustryDefaultUseCase` | 산업 코드 → seed JSON 로드 → nodes_graph 카탈로그 upsert | `user_id: UUID`, `industry_code: str` | `AsyncGenerator[SSEFrame]` | `EmbedderPort`(nodes_graph), `NodeDefinitionRepository.upsert()` |
| `BuildFromFunctionalDomainUseCase` | 직무 영역 코드 → seed JSON 로드 → nodes_graph 카탈로그 upsert | `user_id: UUID`, `domain_code: str` | `AsyncGenerator[SSEFrame]` | `EmbedderPort`(nodes_graph), `NodeDefinitionRepository.upsert()` |
...
| `ModalEmbeddingAdapter` 🆕 | Modal GPU 임베딩 호출 (BGE-M3) | `nodes_graph.domain.ports.EmbedderPort` | `httpx` | composer, personalization |
...
│   └── ports/
│       ├── llm_port.py
│       # EmbedderPort: nodes_graph.domain.ports.EmbedderPort (SSOT — ai_agent 자체 정의 폐기)
