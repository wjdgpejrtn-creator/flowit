...
| 라우터 | 파일 경로 | 엔드포인트 | 설명 |
|--------|-----------|-----------|------|
| `workflows` | `app/routers/workflows.py` | `POST /workflows` | 워크플로우 생성 (WorkflowSchema 기반) |
...
| `agents` | `app/routers/agents.py` | `POST /agents/sessions` | 에이전트 세션 생성 |
...
| `documents` | `app/routers/documents.py` | `POST /documents/upload` | 문서 업로드 |
| | | `GET /documents` | owner 본인 문서 목록 조회 (최신순) — `list[DocumentResponse]`. localStorage SSOT 대체 (#219, PR #234) |
| | | `GET /documents/{id}` | 문서 메타 조회 — `DocumentResponse` (`analysis_status`/`analysis_error`/`analyzed_at` 포함, PR #218) |
| | | `GET /documents/{id}/download` | 다운로드 presigned URL 발급 (TTL 1h) |
| | | `GET /documents/{id}/blocks` | 파싱 결과 본문 조회 — `DocumentBlocksResponse` (분석 완료 후 1회 fetch, PR #218) |
| | | `POST /documents/{id}/analyze` | 문서 분석 요청 — Celery dispatch 후 202, worker가 `analysis_status` 갱신 (PR #218) |
| `auth` | `app/routers/auth.py` | `GET /auth/authorize` | Google OAuth 인증 URL 발급 (state CSRF 토큰 포함) |
| | | `GET /auth/callback` | OAuth 콜백 — HttpOnly 쿠키 set + frontend로 302 (ADR-0021) |
| | | `POST /auth/refresh` | 토큰 갱신 — `refresh_token` 쿠키 기반, 만료 시 401 |
| | | `POST /auth/logout` | 로그아웃 — 세션 revoke + 인증 쿠키 clear |
| | | `GET /auth/me` | 현재 사용자 정보 — `MeResponse` (PermissionSource 필드 + `email`/`name`, 프론트 userName 연결용) |
| `nodes` | `app/routers/nodes.py` | `GET /nodes/catalog` | 노드 카탈로그 조회 (NodeConfig[] — 56종, PR #68) |
| `skills` | `app/routers/skills.py` | `POST /skills/{id}/approve`, `/publish`, `/submit`, `/promote`, `GET /skills/review-queue`, `GET/POST/PUT/DELETE /skills/personal[/{id}][/document]`, `GET /skills/templates`, **`POST /skills/extract`**, **`POST /skills/extract/detail`** | `skills_marketplace` lifecycle (ADR-0020 Q4) + personal CRUD(REQ-013) + 위저드 추출 SSE proxy. extract = wizard 1단계 (`source_type=sop, step=metadata`, SSE, `payload.skill_metas` 5필드), extract/detail = wizard 1.5단계 (`step=detail` + 선택된 meta dict, JSON 응답, `payload.skill_detail`). 옵션 1 2단계 분리(2026-06-04) — LLM JSON 잘림(`max_tokens=4096`) 해소를 위한 추출 토큰 분할 |
| `marketplace` | `app/routers/marketplace.py` | `GET /marketplace`, `POST /marketplace/{id}/promote` | REQ-013 후보 PromoteToTeam/PromoteToCompany |
| `exec_control` | `app/routers/exec_control.py` | `GET /executions/{id}` | REQ-009 폴링용 execution 조회 — `ExecutionGetResponse` (status/started_at/finished_at/error/node_states_summary, `last_event`/`outputs`는 후속 PR) |
| | | `POST /executions/{id}/cancel`, `/resume` | REQ-007 PauseResumeUseCase (Celery revoke + state transition) |
| `health` | `app/routers/health.py` | `GET /health` | DB(`SELECT 1`) + Redis + Orchestrator URL 연결성 점검 |

### 2.3 Services (Application Layer)

| 서비스 | 파일 경로 | 설명 |
|--------|-----------|------|
| `WorkflowService` | `app/services/workflow_service.py` | 워크플로우 CRUD — `WorkflowRepository`(modules/storage) 호출. **save 호출 직전 `permission_source.user_id`를 `WorkflowSchema.owner_user_id`로 명시 주입** (PR #66 v0.3.0, NOT NULL 방어). `WorkflowRepository` ABC 소유권은 **`ai_agent/domain/ports/`** — REQ-007/REQ-009/ai_agent 3곳이 같은 워크플로우 영속화 인터페이스를 공유하므로 ai_agent 도메인이 SSOT 보유 (PR #67 ADR-0012 v3 확정, [[duplicate_code_verify_before_remove]] 패턴). 후속 ADR에서 별도 `workflow` 도메인으로 분리할지 재검토 가능 |
| `SSEStreamService` | `app/services/sse_stream_service.py` | Orchestrator Modal app의 SSE 응답(`text/event-stream`)을 **그대로 클라이언트로 패스스루**. 본 서비스가 LangGraph를 직접 호출하지 않음 |
| `AgentSessionService` | `app/services/agent_session_service.py` | 세션 생성/관리 — Orchestrator HTTP 호출은 `OrchestratorClient` 어댑터 위임. `langgraph_thread_id`는 orchestrator 발급분을 보관 |
| `AuthService` | `app/services/auth_service.py` | JWT 발급/검증, OAuth2 플로우 |

### 2.4 Infrastructure

| 컴포넌트 | 파일 경로 | 설명 |
|----------|-----------|------|
| `SSEEncoder` | `app/infrastructure/sse_encoder.py` | AnySSEFrame → `text/event-stream` 포맷 직렬화 |
| `DependencyContainer` | `app/infrastructure/container.py` | FastAPI Depends 기반 DI 컨테이너 |
| `ErrorHandlers` | `app/infrastructure/error_handlers.py` | 전역 예외 핸들러 (DomainError → HTTP 4xx/5xx) |
| `CORSMiddleware` | `app/infrastructure/middleware.py` | CORS 설정 (프론트엔드 Origin 허용) |
| `RateLimiter` | `app/infrastructure/rate_limiter.py` | 요청 속도 제한 미들웨어 |
...
| 12 | Cloud SQL IAM 인증 표준 ([[sub_agent_cloud_sql_iam]]) | staging/prod에서 `DB_PASSWORD` 평문 DSN 금지. `CLOUD_SQL_INSTANCE` + `DB_IAM_USER`(SA 이메일) + 런타임 토큰 — sub-agent와 동일 환경변수명 |
...
│     `services/api_server/app/adapters/orchestrator_client.py` │
...
services/api_server/
├── app/
│   ├── main.py                          # FastAPI app factory + Uvicorn 실행 (create_app)
│   ├── config.py                        # Settings (BaseSettings) — ORCHESTRATOR_URL, CLOUD_SQL_INSTANCE 등
│   ├── routers/
│   │   ├── __init__.py
...
│   │   ├── agent_session_service.py     # Orchestrator HTTP 위임
│   │   └── auth_service.py
│   ├── adapters/
│   │   └── orchestrator_client.py       # ai_agent Orchestrator HTTP 어댑터 (Modal app)
│   ├── dependencies/
│   │   ├── database.py                  # Cloud SQL IAM async engine ([[sub_agent_cloud_sql_iam]])
│   │   ├── clients.py                   # Redis + httpx.AsyncClient(orchestrator) singleton
│   │   ├── auth.py                      # JWT/Cipher/OAuth/Repos provider
│   │   ├── permission.py                # PermissionSource provider (라우터 의존성)
│   │   ├── repositories.py              # modules/storage Repository 인스턴스 제공
│   │   ├── use_cases.py                 # Use Case provider (Validate, SearchSkills 등)
│   │   ├── orchestrator.py              # OrchestratorClient provider
│   │   └── celery_client.py             # Celery(broker=REDIS_URL).send_task by name (execution_engine import 0건)
│   ├── middleware/
│   │   ├── request_id.py                # X-Request-Id 부여
│   │   ├── cors.py                      # CORSMiddleware factory
│   │   ├── auth.py                      # modules/auth.AuthMiddleware 등록 + _PUBLIC_PATHS wrapper
│   │   ├── error_handler.py             # DomainError → HTTP 매핑
│   │   └── logging.py                   # 구조화 로깅
│   ├── sse/
│   │   └── handler.py                   # Orchestrator text/event-stream 바이트 패스스루
│   └── infrastructure/
...
│   ├── test_sse_passthrough.py
│   └── conftest.py                      # 공용 fixture
├── pyproject.toml
├── Dockerfile                            # python:3.12-slim + modules/* editable install + --factory
└── README.md
```

> `app/domain/topological_scheduler.py`는 생성하지 않는다 (**폐기** — PR #67 단일 소유 정책, execution_engine에서만 보유).
