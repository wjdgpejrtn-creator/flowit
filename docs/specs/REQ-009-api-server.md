...
| 서비스 | 파일 경로 | 설명 |
|--------|-----------|------|
| `WorkflowService` | `app/services/workflow_service.py` | 워크플로우 CRUD — `WorkflowRepository`(modules/storage) 호출. **save 호출 직전 `permission_source.user_id`를 `WorkflowSchema.owner_user_id`로 명시 주입** (PR #66 v0.3.0, NOT NULL 방어) |
