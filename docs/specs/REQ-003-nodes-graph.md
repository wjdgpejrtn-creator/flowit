...
class NodeDefinition:
    """54종 노드 타입의 카탈로그 엔티티 (gemma_chat 추가 PR #68 / llm_judge 추가 #438).
    ...
    is_mvp: bool                            # MVP 54종 여부 (gemma_chat·llm_judge 포함)
    ...
    # === ADR-0020 (i) scope 격리 ===
    owner_user_id: Optional[UUID] = None    # None=company 전역(기존 54종) / 값=personal 스킬 소유자
...
> 호출한다 (`ToolsetExecutor` 경로 폐기). **external 26종 + domain 28종 = 54종
...
├── adapters/
│   └── catalog/
│       ├── external/                # 26종 NodeDefinition (기존 14 + 신규 11 REQ-005 연동 + llm_judge #438)
...
>
> 박아름 1주차 작업분 41종(28 domain + 13 external) + 박아름 5/14 야간 추가 `gemma_chat` 1종 (PR #68) + 박아름 5/19 toolset 정리 PR로 REQ-005 toolset 연동 11종을 `adapters/catalog/external/`로 이전 + #438 §6.6 품질 루프 scorer `llm_judge` 1종 = **합계 54종**.
>
