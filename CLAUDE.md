...
| ai_agent | skills_marketplace의 `domain/ports` | `from skills_marketplace.domain.ports import SkillOntologyProjector` (`Neo4jSkillProjector`가 게시 스킬 `(:Skill)-[:BINDS]->(:Node)` 투영 시 ABC 구현 — ADR-0026 Phase 2b. EmbedderPort 예외 패턴: Neo4j 호출 어댑터를 호출 모듈 ai_agent가 보유, Port는 소비자 skills_marketplace 소유) |
| ai_agent | skills_marketplace의 `domain/ports` | `from skills_marketplace.domain.ports import SkillDocumentStore` (Composer `_drafter_node`가 선택된 게시 스킬의 COMPOSER.md `composer_instructions`를 `load()`해 drafter에 주입 — ADR-0024 D5 런타임 주입, #372 결함 A 해소. Port ABC만 참조 + 구현체 `GcsSkillDocumentStore`는 composition root 주입. execution_engine SkillDocumentStore 행과 동일 패턴) |
| ai_agent | doc_parser의 `application/use_cases` | `from doc_parser.application.use_cases import ParseDocumentUseCase` (Skills Builder T2 `parse_document` 툴 `ParseUserDocumentUseCase`가 업로드 SOP를 DocumentBlock으로 파싱 시 — ADR-0028 D1/O7. 원시 `ParserPort` 대신 use case를 래핑하는 이유 = mime 디스패치+정규화+**PII 마스킹**+품질게이트 파이프라인 재사용으로 SOP 개인정보가 LLM 추출로 새는 보안 회귀 차단. 구현체는 composition root 주입. doc_parser 오너 김진형(REQ-006) 통지) |
...
| `ai_agent/domain/ports/SlotMapperPort` | `ai_agent/adapters/llm/` (`LlmSlotMapper` — Gemma 슬롯→노드 방향성 매핑, 앙상블 LLM voter. ADR-0026 §6.6 Phase 2. EmbedderPort와 동일 — Modal 호출 어댑터는 ai_agent 보유) |
...
| `doc_parser/domain/ports/DocumentRepositoryPort` | `storage/repositories/` (`PgDocumentRepository` — `save`/`get_by_id`/`save_chunks`/`get_chunks`/`delete`. `get_chunks`는 SOP→스킬 추출 청크 map-reduce/RAG가 소비, PR #483) |
...
| `ai_agent/domain/ports/OntologyRetrieverPort` | `ai_agent/adapters/ontology/` (`Neo4jOntologyAdapter` — Neo4j AuraDB GraphRAG, 요청마다 driver, `NEO4J_*` env. ADR-0026 Phase 1, PR #393) |
| `ai_agent/domain/ports/DomainGroundingPort` | `ai_agent/adapters/ontology/` (`Neo4jDomainGroundingAdapter` — skill builder 전용 도메인 서브그래프(`:Domain`/`:Playbook`/`:Stage`/`:Rule`)만 질의, composer `OntologyRetrieverPort`와 **완전 별개**(라벨/쿼리/어댑터 disjoint). 요청마다 driver, `NEO4J_*` env. EmbedderPort 예외 패턴(Neo4j 호출 어댑터는 호출 모듈 ai_agent 보유). ADR-0029 P0, PR #503) |
| `skills_marketplace/domain/ports/SkillOntologyProjector` | `ai_agent/adapters/ontology/` (`Neo4jSkillProjector` — 게시 시 `(:Skill)-[:BINDS]->(:Node)` incremental upsert, 요청마다 driver, `NEO4J_*` env. **예외 패턴** — Port 소유 skills_marketplace(소비=publish)지만 Neo4j 호출 어댑터는 ai_agent 보유(ADR-0013 EmbedderPort와 동일). 모델 A: ai 노드 + required_connections 노드 BINDS. ADR-0026 Phase 2b) |
| `ai_agent/domain/ports/UserDocumentSearchPort` | `storage/repositories/` (**후속 미구현** — `document_chunks` pgvector HNSW 쿼리 + `documents` join으로 file_name + `user_id` 스코프 IDOR 차단. 조장 후속, ADR-0028 T1 — `AgentMemoryRepository`/`SkillRepository`와 동일 분담) |
